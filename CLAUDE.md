...
Beacon's maintained application is the FastAPI + React desktop stack. The old root-level Flask engine has been retired. The execution engine has one source of truth under `backend/app/core/`; do not create or synchronize a second copy.

- `backend/app/main.py` — FastAPI backend.
- `backend/app/core/` — models, templating, transport, assertions, extraction, metrics, and execution orchestration.
...
Python regression tests live in `backend/tests/` and run with `python -m pytest backend/tests` after installing backend requirements.

## Config persistence

The backend resolves its writable data directory through `backend/app/paths.py`, including desktop sidecar/runtime locations. Do not add cwd-relative persistence paths. Config mutations belong in the backend state/services layer.

## Core engine architecture (`backend/app/core/`)

- **`models.py`** — `EndpointTest`, `TestConfig`, and their persisted JSON contract.
- **`templating.py`** — recursive variables and fresh-per-request generators.
- **`transport.py`** — HTTP request construction and dispatch.
- **`assertions.py`** — response assertion evaluation.
- **`extractors.py`** — response value and cookie extraction.
- **`metrics.py`** — counters, latency samples, and snapshots.
- **`tester.py`** — the `APITester` orchestration facade and traffic modes.
...
`POST /run` starts `APITester` execution and returns a `run_id`. Progress is exposed through status APIs and WebSocket events. Active runs are runtime state and are lost on backend restart; durable summaries go through the history layer.
